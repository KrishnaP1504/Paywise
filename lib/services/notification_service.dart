...
class NotificationService {
  ...
  Future<void> init() async {
    ...
    // Prompt native permissions post-frame so the first UI frame renders without delay
  ...
  Future<void> scheduleAll5EmiReminders({
    ...
    DateTime overdueCutoff = DateTime(targetYear, targetMonth, safeDueDay + 1, 11, 0);
    if (overdueCutoff.isBefore(now)) {
      // Advance to next month only if the overdue alert time for this month has already passed
      targetMonth += 1;
      if (targetMonth > 12) {
        targetMonth = 1;
        targetYear += 1;
    ...
    if (!masterEnabled) return; // All notifications disabled by user
    ...
    if (notifyAdvance && date1.isAfter(now)) {
    ...
    if (notifyDueTomorrow && date2.isAfter(now)) {
      await flutterLocalNotificationsPlugin.zonedSchedule(
    ...
    if (notifyDueToday && date3.isAfter(now)) {
      await flutterLocalNotificationsPlugin.zonedSchedule(
    ...
    if (notifyAdvance && date4.isAfter(now)) {
      await flutterLocalNotificationsPlugin.zonedSchedule(
    ...
    if (notifyOverdue && date5.isAfter(now)) {
      await flutterLocalNotificationsPlugin.zonedSchedule(
