        ...
        bottom: PreferredSize(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isDark
                  ? const Color(0xFF1B1D30).withValues(alpha: 0.70)
            ...
            child: TabBar(
              ...
              unselectedLabelColor: isDark ? Colors.grey[400] : const Color(0xFF475569),
                ...
                Tab(
                  ...
                  child: FittedBox(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.published_with_changes_rounded, size: 16),
                        SizedBox(width: 5),
                        Text('Refinancing'),
                      ],
                    ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${selectedLoan.interestRate}% interest · ${selectedLoan.tenureMonths}m · ${AppCurrency.format(selectedLoan.emiAmount)}/mo',
                    style: TextStyle(
                      fontSize: 12,
      ...
      builder: (context) {
        return ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: Container(
              decoration: GlassTheme.bottomSheetDecoration(context, radius: 24),
                          ...
                          child: Container(
                            ...
                            decoration: GlassTheme.cardDecoration(
                              ...
                              customBorder: isSelected
                                  ? Border.all(color: Colors.indigo, width: 1.8)
                                  : null,
                            ),
                            child: Row(
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 2),
                                      Text(
                                        '${loan.interestRate}% rate · ${loan.tenureMonths} months · ${AppCurrency.format(loan.emiAmount)}/mo',
                                        style: TextStyle(
                                          fontSize: 12,
                                          color: isDark
                                              ? Colors.grey[400]
                                ...
                                Text(
                                  AppCurrency.format(loan.principalAmount),
                                  style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    fontSize: 13,
                                    color: isSelected
...
class _ExtraEmiTabState extends State<_ExtraEmiTab> {
  ...
  double get _minExtra => min(100.0, max(50.0, (widget.loan.emiAmount * 0.1).roundToDouble()));
  double get _maxExtra => max(_minExtra + 100.0, (widget.loan.emiAmount * 3).roundToDouble());

  @override
  void initState() {
    super.initState();
    _extraAmount = (widget.loan.emiAmount * 0.5).clamp(_minExtra, _maxExtra);
    _extraCtrl = TextEditingController(text: AppCurrency.format(_extraAmount, showSymbol: false));
  }
  ...
  void didUpdateWidget(_ExtraEmiTab old) {
    ...
    if (old.loan != widget.loan) {
      _extraAmount = (widget.loan.emiAmount * 0.5).clamp(_minExtra, _maxExtra);
      _startingMonth = 1;
      _extraCtrl.text = AppCurrency.format(_extraAmount, showSymbol: false);
      _extraError = null;
    }
  }
  ...
  void _applyExtraFromText(String raw) {
    ...
    setState(() {
      if (val == null || val < _minExtra) {
        _extraError = 'Minimum extra payment is ${AppCurrency.format(_minExtra)}';
        _extraAmount = _minExtra;
        _extraCtrl.text = AppCurrency.format(_minExtra, showSymbol: false);
      } else if (val > _maxExtra) {
        _extraError = 'Max allowed is ${AppCurrency.format(_maxExtra)} (3× EMI)';
        _extraAmount = _maxExtra;
        _extraCtrl.text = AppCurrency.format(_maxExtra, showSymbol: false);
      } else {
        _extraError = null;
        _extraAmount = val;
      }
  ...
  double? _lastExtraAmount;
  int? _lastStartingMonth;
  LoanModel? _lastLoan;

  Map<String, dynamic> _getCalculatedResult() {
    if (_cachedResult != null &&
        _lastExtraAmount == _extraAmount &&
        _lastStartingMonth == _startingMonth &&
        _lastLoan == widget.loan) {
      return _cachedResult!;
    }
    _lastExtraAmount = _extraAmount;
    _lastStartingMonth = _startingMonth;
    _lastLoan = widget.loan;
    _cachedResult = _calculate();
    return _cachedResult!;
  }
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.info_outline_rounded, color: Colors.indigo, size: 30),
              ),
              const SizedBox(height: 16),
              const Text(
                "Extra EMI Not Applicable",
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 8),
              Text(
                "For loans with a very short tenure (${loan.tenureMonths} months) or small principal (${AppCurrency.format(loan.principalAmount)}), Extra EMI simulation is not needed because any prepayment will immediately clear the loan in full.",
                style: const TextStyle(fontSize: 13.5, color: Colors.grey, height: 1.45),
                textAlign: TextAlign.center,
              ),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        "Tip: Use the Lump Sum tab to simulate paying off the remaining ${AppCurrency.format(loan.outstandingBalance)} in one go!",
                        style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w500, color: Color(0xFF2E7D32)),
                      ),
                    ),
                  ],
                ),
            ...
            child: Column(
              ...
              children: [
                ...
                _AmountField(
                  ...
                  color: accentColor,
                  hint: 'e.g. ${AppCurrency.format(minExtra, showSymbol: false)}',
                  min: minExtra,
                  max: maxExtra,
                ...
                Slider(
                  value: _extraAmount.clamp(minExtra, maxExtra),
                  min: minExtra,
                  max: maxExtra,
                  divisions: max(1, ((maxExtra - minExtra) / 50).round().clamp(1, 300)),
                  activeColor: accentColor,
                  onChanged: (v) {
                    setState(() {
                      _extraAmount = v;
                      _extraError = null;
                      _extraCtrl.text = AppCurrency.format(v, showSymbol: false);
                    });
                  },
                ),
                _RangeHint(
                  min: '${AppCurrency.format(minExtra)} (min)',
                  max: '${AppCurrency.format(maxExtra)} (3× EMI)',
                ),
                const SizedBox(height: 16),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                ...
                Slider(
                  value: _startingMonth.toDouble().clamp(1.0, max(2.0, loan.tenureMonths.toDouble())),
                  min: 1,
                  max: max(2.0, loan.tenureMonths.toDouble()),
...
class _LumpSumTabState extends State<_LumpSumTab> {
  ...
  double get _minLump => min(500.0, max(50.0, (widget.loan.outstandingBalance * 0.1).roundToDouble()));
  double get _maxLump => max(_minLump + 50.0, widget.loan.outstandingBalance);

  @override
  void initState() {
    super.initState();
    _singleAmount = (_maxLump * 0.5).clamp(_minLump, _maxLump);
    _singleMonth = min(6, max(1, widget.loan.tenureMonths));
    _annualAmount = (_maxLump * 0.3).clamp(_minLump, _maxLump);
    _annualMonth = min(5, max(1, widget.loan.tenureMonths));
    _customEntries.add(
      CustomLumpSumEntry(
        month: min(12, max(1, widget.loan.tenureMonths)),
        amount: (_maxLump * 0.5).clamp(_minLump, _maxLump),
      ),
    );
  ...
  void didUpdateWidget(_LumpSumTab old) {
    ...
    if (old.loan != widget.loan) {
      _singleAmount = (_maxLump * 0.5).clamp(_minLump, _maxLump);
      _singleMonth = min(6, max(1, widget.loan.tenureMonths));
      _singleCtrl.text = AppCurrency.format(_singleAmount, showSymbol: false);
      _singleError = null;

      _annualAmount = (_maxLump * 0.3).clamp(_minLump, _maxLump);
      _annualMonth = min(5, max(1, widget.loan.tenureMonths));
      _annualCtrl.text = AppCurrency.format(_annualAmount, showSymbol: false);
      _annualError = null;
    }
  }
  ...
  void _applySingleFromText(String raw) {
    final val = double.tryParse(raw.replaceAll(',', ''));
    setState(() {
      if (val == null || val < _minLump) {
        _singleError = 'Minimum lump sum is ${AppCurrency.format(_minLump)}';
        _singleAmount = _minLump;
        _singleCtrl.text = AppCurrency.format(_minLump, showSymbol: false);
      } else if (val > _maxLump) {
        _singleError = 'Cannot exceed balance (${AppCurrency.format(_maxLump)})';
        _singleAmount = _maxLump;
        _singleCtrl.text = AppCurrency.format(_maxLump, showSymbol: false);
      } else {
        _singleError = null;
        _singleAmount = val;
      }
  ...
  void _applyAnnualFromText(String raw) {
    ...
    setState(() {
      if (val == null || val < _minLump) {
        _annualError = 'Minimum bonus is ${AppCurrency.format(_minLump)}';
        _annualAmount = _minLump;
        _annualCtrl.text = AppCurrency.format(_minLump, showSymbol: false);
      } else if (val > _maxLump) {
        _annualError = 'Cannot exceed balance (${AppCurrency.format(_maxLump)})';
        _annualAmount = _maxLump;
        _annualCtrl.text = AppCurrency.format(_maxLump, showSymbol: false);
      } else {
        _annualError = null;
        _annualAmount = val;
      }
  ...
  void _showAddCustomDialog() {
    int selectedMonth = min(12, max(1, widget.loan.tenureMonths));
    final defaultAmt = min(widget.loan.outstandingBalance, max(100.0, _minLump));
    final amountCtrl = TextEditingController(text: AppCurrency.format(defaultAmt, showSymbol: false));
    final double maxMonth = max(2.0, widget.loan.tenureMonths.toDouble());
        ...
        return StatefulBuilder(
          builder: (context, setDlgState) {
            return GlassAlertDialog(
              icon: const Icon(Icons.add_card_rounded, color: Colors.indigo, size: 28),
              ...
              content: Column(
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text(
                        "Pay in Month $selectedMonth (Year ${(selectedMonth / 12).ceil()})",
                        style: const TextStyle(fontWeight: FontWeight.w600),
                      ),
                      Slider(
                        value: selectedMonth.toDouble().clamp(1.0, maxMonth),
                        min: 1,
                        max: maxMonth,
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    final amt = AppCurrency.parseClean(amountCtrl.text);
                    if (amt != null && amt > 0) {
                      setState(() {
  ...
  Map<String, dynamic>? _cachedResult;
  LumpSumMode? _lastMode;
  double? _lastSingleAmount;
  int? _lastSingleMonth;
  double? _lastAnnualAmount;
  int? _lastAnnualMonth;
  int? _lastCustomLength;
  double? _lastCustomSum;
  LoanModel? _lastLoan;

  Map<String, dynamic> _getCalculatedResult() {
    final customSum = _customEntries.fold(0.0, (sum, p) => sum + p.amount);
    if (_cachedResult != null &&
        _lastMode == _mode &&
        _lastSingleAmount == _singleAmount &&
        _lastSingleMonth == _singleMonth &&
        _lastAnnualAmount == _annualAmount &&
        _lastAnnualMonth == _annualMonth &&
        _lastCustomLength == _customEntries.length &&
        _lastCustomSum == customSum &&
        _lastLoan == widget.loan) {
      return _cachedResult!;
    }
    _lastMode = _mode;
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                _buildModeBtn("One-Time", LumpSumMode.single, Icons.bolt_rounded, isDark),
                _buildModeBtn("Yearly Bonus", LumpSumMode.annual, Icons.calendar_month_rounded, isDark),
                _buildModeBtn("Custom Multi", LumpSumMode.custom, Icons.tune_rounded, isDark),
              ],
            ),
          ),
          const SizedBox(height: 16),
          ...
          AnimatedSwitcher(
            ...
            child: _InsightBanner(
              key: ValueKey('insight_${_mode.name}'),
              color: accentColor,
  ...
  Widget _buildModeBtn(String title, LumpSumMode mode, IconData icon, bool isDark) {
  ...
  Widget _buildSingleModeCard(LoanModel loan, Color accentColor) {
    return _SimCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Slider(
            value: _singleAmount.clamp(_minLump, _maxLump),
            min: _minLump,
            max: _maxLump,
            divisions: max(1, ((_maxLump - _minLump) / 100).round().clamp(1, 500)),
            activeColor: accentColor,
            onChanged: (v) {
              setState(() {
                _singleAmount = v;
                _singleError = null;
                _singleCtrl.text = AppCurrency.format(v, showSymbol: false);
              });
            },
          ),
          _RangeHint(
            min: '${AppCurrency.format(_minLump)} (min)',
            max: '${AppCurrency.format(_maxLump)} (full balance)',
          ),
          const SizedBox(height: 16),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
          ...
          Slider(
            value: _singleMonth.toDouble().clamp(1.0, max(2.0, loan.tenureMonths.toDouble())),
            min: 1,
            max: max(2.0, loan.tenureMonths.toDouble()),
            divisions: max(1, loan.tenureMonths - 1),
            activeColor: accentColor,
  ...
  Widget _buildAnnualModeCard(LoanModel loan, Color accentColor) {
    return _SimCard(
      ...
      child: Column(
        ...
        children: [
          ...
          _AmountField(
            ...
            color: accentColor,
            hint: 'e.g. ${AppCurrency.format(_minLump, showSymbol: false)}',
            min: _minLump,
            max: _maxLump,
          ...
          Slider(
            value: _annualAmount.clamp(_minLump, _maxLump),
            min: _minLump,
            max: _maxLump,
            divisions: max(1, ((_maxLump - _minLump) / 100).round().clamp(1, 500)),
            activeColor: accentColor,
            onChanged: (v) {
              setState(() {
                _annualAmount = v;
                _annualError = null;
                _annualCtrl.text = AppCurrency.format(v, showSymbol: false);
              });
            },
          ),
          _RangeHint(
            min: '${AppCurrency.format(_minLump)} (min)',
            max: '${AppCurrency.format(_maxLump)} (max)',
          ),
          const SizedBox(height: 16),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
          ...
          Center(
            child: Container(
              ...
              child: Text(
                'Selected: ${DateFormat('MMMM').format(DateTime(2026, _annualMonth))} (Month $_annualMonth)',
                style: TextStyle(
                  fontSize: 12.5,
                  fontWeight: FontWeight.bold,
                  color: accentColor,
  ...
  Widget _buildCustomModeCard(LoanModel loan, Color accentColor, bool isDark, NumberFormat currency) {
    return _SimCard(
...
class _RefinancingTabState extends State<_RefinancingTab> {
  ...
  double get _maxRate => max(1.1, widget.loan.interestRate - 0.01);
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: isDark ? Colors.indigo.withValues(alpha: 0.2) : const Color(0xFFEEF0FD),
                  shape: BoxShape.circle,
                ),
                child: const Icon(Icons.percent_rounded, size: 48, color: Colors.indigo),
              ),
              ...
              const SizedBox(height: 8),
              Text(
                'This loan already has an ultra-low interest rate (${loan.interestRate}%). Refinancing is designed for lowering high interest rates and is not applicable here.',
                textAlign: TextAlign.center,
          ...
          _SimCard(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: _RateBox(
                        ...
                        color: isDark ? const Color(0xFFEF4444).withValues(alpha: 0.16) : const Color(0xFFFFEBEE),
                    ...
                    Expanded(
                      child: _RateBox(
                        label: 'New Rate',
                        value: '${_newRate.toStringAsFixed(2)}%',
                        color: isDark ? const Color(0xFF10B981).withValues(alpha: 0.16) : const Color(0xFFE8F5E9),
                        textColor: isDark ? const Color(0xFF6EE7B7) : successColor,
                      ),
                    ),
                  ],
                ),
                ...
                Slider(
                  value: _newRate.clamp(_minRate, safeMaxRate),
                  min: _minRate,
                  max: safeMaxRate,
                  divisions: max(1, ((safeMaxRate - _minRate) * 10).round().clamp(1, 200)),
                  activeColor: accentColor,
                  onChanged: (v) {
                    setState(() {
                      _newRate = v;
                ...
                _RangeHint(min: '${_minRate.toStringAsFixed(1)}% (min)', max: '${safeMaxRate.toStringAsFixed(2)}% (just below current)'),
              ],
            ),
          ),
          const SizedBox(height: 16),
...
class _PillBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: GlassTheme.pillDecoration(context, color: color, radius: 20),
